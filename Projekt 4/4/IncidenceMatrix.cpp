...
void IncidenceMatrix::deleteEdge(int node1, int node2)
{
	for (int i = 0; i < edges; i++)
	{
		if (Representation[node1][i] == 1 && Representation[node2][i] == -1){
			Representation[node1][i] = 0;
			Representation[node2][i] = 0;
			break;
		}
	}
}
...
void IncidenceMatrix::print()
{
	...
	for (int i = 0; i < nodes; i++)
	{
		...
		{
			if(Representation[i][j] == 1 || Representation[i][j] == 0)
...
int IncidenceMatrix::getRepresentation(int a, int b){
